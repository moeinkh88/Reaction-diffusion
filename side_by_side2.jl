# Reference for new parameters: Pearson, J. E. (1993). Complex Patterns in a Simple System. Science, 261(5118), 189-192. (Specifically, the $\epsilon$ class of patterns).
using FdeSolver
using Plots
...
# Changed to Pearson's Labyrinth/Stripe Regime
D1, D2, F_rate, k_rate = 0.16, 0.08, 0.040, 0.060
tSpan = [0.0, 600.0] # Increased time span for labyrinth formation
dt = 0.1 
...
# Morphogen 1 (u1) has strong memory (0.8)
β_u1 = fill(0.8, N^2)

# Morphogen 2 (u2) has weak memory (1.0)
β_u2 = fill(0.9, N^2)
...
# Updated colormap to :viridis for clear scientific contrast
...
p_end_nomem   = heatmap(u1_end_nomem; title="No Memory (t=$(tSpan[2]))", hm_args...)
p_start_mem   = heatmap(u1_start; title="With Memory (t=0)", hm_args...)
p_end_mem     = heatmap(u1_end_mem; title="With Memory (t=$(tSpan[2]))", hm_args...)
...
for i in 1:save_interval:num_steps
    ...
    # Plot side-by-side with updated colormap
